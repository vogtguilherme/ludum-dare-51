...
public class IKFootPlacement : MonoBehaviour
{
    ...
    [SerializeField] float RayExtraLength = 1f;
    ...
    private void OnAnimatorIK(int layerIndex)
    {
        ...
        { // Only carry out the following code if there is an Animator set.
            ...
            if (Physics.Raycast(ray, out hit, DistanceToGround + RayExtraLength, layerMask))
            {
                ...
                if (hit.transform.tag == "Walkable")
                {
                    ...
                    anim.SetIKRotation(AvatarIKGoal.LeftFoot, Quaternion.LookRotation(Vector3.Cross(transform.right, hit.normal), hit.normal));
            ...
            if (Physics.Raycast(ray, out hit, DistanceToGround + RayExtraLength, layerMask))
            {
                if (hit.transform.tag == "Walkable")
                {
                    ...
                    anim.SetIKRotation(AvatarIKGoal.RightFoot, Quaternion.LookRotation(Vector3.Cross(transform.right, hit.normal), hit.normal));
                }
            }
        }
    }
